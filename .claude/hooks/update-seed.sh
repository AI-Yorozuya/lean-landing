#!/bin/bash
# 回訪自動更新（SessionStart hook）：把「種子的東西」同步到 main 最新，玩家的東西一律不碰。
...
# 抓不到網路→靜靜用本地版，永不擋開演。Windows（Git Bash）待實測。
set -u
cd "$(dirname "$0")/../.." || exit 0
TMP="$(mktemp -d)" || exit 0
trap 'rm -rf "$TMP"' EXIT
...
if curl -fsSL --max-time 10 "$URL" -o "$TMP/seed.tgz" 2>/dev/null \
  ...
  echo "種子已同步最新版（劇本與知識更新了；你的網站 site/ 與進度檔一個字都沒動）。"
else
  echo "（離線或更新抓取失敗——用本地版劇本繼續，不影響遊玩。）"
fi
exit 0
