...
- **取種規則（固定，不即興）**：目前資料夾是空的→`git clone <repo> .` **直接攤開**——玩家選的資料夾就是他的店，不多包一層；不是空的→clone 成子資料夾再進去。抓完**斷臍帶**：`rm -rf .git && git init && git add -A && git commit -m "種子就位"`——歷史從玩家開始，也不留指向種子 repo 的 origin（不然之後備份 `gh repo create --source=. --push` 會撞 remote）。這是唯一允許的 `rm -rf .git`，僅限剛抓完種子這一刻。
